...
/// <summary>
/// SFT-T41: тест-кейси для auto-update flow.
///
/// Покриває:
///   - GetUpdateMode: None / Optional / Force
///   - IsNewer: базові випадки (вже в UpdateServiceTests.cs, тут edge cases)
///   - VersionInfo десеріалізація (через System.Text.Json)
/// </summary>
public class AutoUpdateTests
{
    ...
    {
        // Assembly versions can have 4 parts: 1.4.7.0 > 1.4.7 → false (equal)
    ...
    {
        // All of these represent 1.0.0 which equals or is older than current in tests
        var cur = new Version(2, 0, 0);
        // GetUpdateMode with explicit current is the safe way to test
    ...
    // ── VersionInfo JSON deserialization ───────────────────────────────────────
    ...
    // ── Scenario: typical CI/CD release flow ──────────────────────────────────

    [Fact]
    public void Scenario_CicdRelease_OldClientSeesOptionalUpdate()
    {
        // CI pusher → backend оновив версію до 1.4.7, force=false
    ...
    {
        // Адмін поставив force_update=true для критичного фіксу
